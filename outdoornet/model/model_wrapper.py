...
from outdoornet.dataandgeo.transforms import get_transforms
from outdoornet.dataandgeo.utils import inv2depth, post_process_inv_depth, compute_depth_metrics
from outdoornet.dataandgeo.utils import print0, world_size, rank, on_rank_0
from outdoornet.dataandgeo.utils import flip_lr
from outdoornet.dataandgeo.utils import load_class, load_class_args_create, \
    load_network, filter_args
from outdoornet.dataandgeo.utils import pcolor
from outdoornet.dataandgeo.utils import all_reduce_metrics, create_dict, average_loss_and_metrics
from outdoornet.dataandgeo.utils import save_depth
from outdoornet.models.model_utils import stack_batch


class ModelWrapper(torch.nn.Module):
    """
    Top-level torch.nn.Module wrapper around a SfmModel (pose+depth networks).
    Designed to use model with high-level Trainer classes (cf. train/).
    ...
    def __init__(self, config, resume=None, logger=None, load_datasets=True):
        ...
        # Model, optimizers, schedulers and dataandgeo are None for now
...
def setup_depth_net(config, prepared, **kwargs):
    ...
    depth_net = load_class_args_create(config.name,
        paths=['outdoornet.networks.depth',],
        args={**config, **kwargs},
    )
...
def setup_pose_net(config, prepared, **kwargs):
    ...
    pose_net = load_class_args_create(config.name,
        paths=['outdoornet.networks.pose',],
        args={**config, **kwargs},
    )
    if not prepared and config.checkpoint_path is not '':
...
def setup_model(config, prepared, **kwargs):
    ...
    model = load_class(config.name, paths=['outdoornet.model',])(
        **{**config.loss, **kwargs})
...
def setup_dataloader(datasets, config, mode):
    ...
    datasets : list of Dataset
        List of dataandgeo from which to create dataloaders
    config : CfgNode
        Model configuration (cf. configs/default_config.py)
    mode : str {'train', 'validation', 'test'}
