...
from outdoornet.dataandgeo.utils import s3_url, prepare_dataset_prefix
from outdoornet.dataandgeo.utils import on_rank_0
from outdoornet.dataandgeo.utils.types import is_cfg, is_list
from outdoornet.dataandgeo.utils import make_list
from outdoornet.dataandgeo.utils import load_class, backwards_state_dict
...
def prep_logger_and_checkpoint(model):
    ...
    if model.logger and not model.config.wandb.dry_run:
        ...
        # If we are saving model we need to update the path
...
def backwards_config(config):
    ...
    (no need for it right now, pretrained model are up-to-date with configuration files).
...
def prepare_test_config(config):
    ...
    config : CfgNode
        Prepared model configuration
    """
    # Remove train and validation dataandgeo
    config.datasets.train.path = config.datasets.validation.path = []
    config.datasets.test = prep_dataset(config.datasets.test)
    # Don't save model or log to wandb
